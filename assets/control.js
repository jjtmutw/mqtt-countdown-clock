...
function handleIncomingMessage(topic, message) {
  ...
  try {
    ...
    setDisplayClock(payload.display || formatDisplayTime(payload.remainingSeconds), false);
...
window.setInterval(() => {
  ...
  setDisplayClock(els.displayClock.textContent, Date.now() - state.displayLastSeenAt > 4000);
}, 1000);
